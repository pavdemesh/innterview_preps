...
Friends decided that they will implement a problem if at least two of them are sure about the solution.
...
# Initialize counter for solvable problems and set it to 0
counter_solvable = 0

# For the (total number of problems) times: read input, update counter for solvable problems if sum of  sureness >= 2
for i in range(num_of_problems):
    who_is_sure = list(map(int, input().split()))
    if sum(who_is_sure) >= 2:
        counter_solvable += 1

# Display result
print(counter_solvable)
