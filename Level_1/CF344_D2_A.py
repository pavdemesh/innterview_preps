...
The opposite poles will attract each other.
...
Then n lines follow. The i-th line (1 <= i <= n) contains
either characters "01", if Mike put the i-th magnet in the "plus-minus" position,
...
# Initialize polarity to None and count of groups to 0
polarity = None
count_groups = 0

# Get n lines of input.
# If current magnet differs from current polarity - change polarity and increase count of groups
for i in range(n):
    curr_magnet = input()
    if curr_magnet == polarity:
        continue
    else:
        count_groups += 1
        polarity = curr_magnet
